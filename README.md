...
未实现的模式在 `examples/` 下只有可编译骨架；`singleton`、`factory_method`、`abstract_factory`、`builder`、`prototype`、`adapter`、`decorator`、`proxy`、`facade`、`composite`、`bridge`、`flyweight`、`strategy`、`observer` 和 `template_method` 是真实使用场景。详见 [`examples/README.md`](examples/README.md)。

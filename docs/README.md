...
客户端示例在 [`examples/`](../examples/README.md)：未实现的模式是程序骨架，已实现的 `singleton` / `factory_method` / `abstract_factory` / `builder` / `prototype` / `adapter` / `decorator` / `proxy` / `facade` / `composite` / `bridge` 是真实用法。
